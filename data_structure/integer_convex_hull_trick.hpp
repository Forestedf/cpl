...
class IntegerConvexHullTrick {
    ...
    inline bool need(const std::pair<T, T> &l0, const std::pair<T, T> &l1, const std::pair<T, T> &l2) const {
...
public:
    ...
    void add(T a, T b) {
        ...
        std::pair<T, T> l(a, b);
        while (lines.size() >= 2 && !need(lines[lines.size() - 2], lines.back(), l)) {
            lines.pop_back();
        }
    ...
    friend IntegerConvexHullTrick<T> merge_two_chts(const IntegerConvexHullTrick<T> &cht0, const IntegerConvexHullTrick<T> &cht1) {
        Compare comp;
        ...
        while (it0 != cht0.lines.end() || it1 != cht1.lines.end()) {
            if (it1 == cht1.lines.end() || (it0 != cht0.lines.end() && !comp(it0->first, it1->first))) {
